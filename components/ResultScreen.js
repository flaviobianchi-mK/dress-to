import { categoryLabel, formatPrice, pieceNeedsSize } from '../js/catalog-data.js';
...
export default {
  ...
  emits: ['copy-image', 'copy-ref', 'copy-all-refs', 'regenerate', 'restart', 'back'],
  setup(props) {
          ...
          <div class="dt-result__panel dt-glass-2">
            <div class="dt-result__panel-head">
              ...
              <button
                ...
                :class="copiedAllRefs ? 'dt-btn--primary is-success' : 'dt-btn--outline'"
                @click="$emit('copy-all-refs')"
              >
                {{ copiedAllRefs ? 'Referências copiadas' : 'Copiar todas as referências' }}
              </button>
            </div>
              ...
              <li v-for="piece in pieces" :key="piece.id" class="dt-ref-row">
                ...
                <div>
                  <div class="dt-ref-row__ref">
                    {{ piece.ref }}<template v-if="pieceNeedsSize(piece)"> · Tam. {{ piece.size }}</template>
                  </div>
